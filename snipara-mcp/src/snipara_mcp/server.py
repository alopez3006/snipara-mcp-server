...
def _load_auth() -> tuple[str | None, str, str | None]:
    ...
    if not ignore_oauth:
        try:
            ...
            tokens = load_tokens()
            if tokens:
                # If PROJECT_ID or PROJECT_SLUG is set, find matching token
                env_project_id = os.environ.get("SNIPARA_PROJECT_ID")
                env_project_slug = os.environ.get("SNIPARA_PROJECT_SLUG")
                ...
                # Then try matching by project_slug
                if env_project_slug or env_project_id:
                    target_slug = env_project_slug or env_project_id
                    for project_id, token_data in tokens.items():
                        if token_data.get("project_slug") == target_slug:
                            access_token = token_data.get("access_token")
                            if access_token:
                    ...
                    "memories": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "category": {"type": "string", "description": "Optional category for grouping"},
                                "ttl_days": {"type": "integer", "description": "Days until expiration"},
                                "related_to": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                    "description": "IDs of related memories",
...
tool usage breakdown, daily trends, and error analysis for the specified time period.""",
            inputSchema={
                "type": "object",
                "properties": {
                    "days": {
